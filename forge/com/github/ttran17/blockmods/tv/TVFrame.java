...
public class TVFrame extends BlockContainer {

	private static int tvFrameID = -1;
	
	public static void setDummyID(TVFrame block) {
		tvFrameID = block.blockID;
	}
	 ...
	 * @return
	 */
	public static int getTVFrameID() {
		if (tvFrameID == -1) {
	...
	public TVFrame(int blockID, Material material) {
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int x, int y, int z)
    {
		...
		if (tileEntity != null) {
			...
			if (block != null && block instanceof TVMultiBlock) {
				double[] bound = ((TVMultiBlock) block).getBlockBoundsBasedOnState(world, tileEntity.tv_x, tileEntity.tv_y, tileEntity.tv_z);
